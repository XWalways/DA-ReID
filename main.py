...
class Main():
    def __init__(self, model, loss):
        ...
        if opt.stage == 1:
            ...
            self.train_loader = train_loader_woEr
    ...
    def evaluate(self, save_path):
        ...
        def rank(dist):
            r = cmc(dist, self.dataset.query_pids, self.dataset.gallery_pids, self.dataset.query_cams, self.dataset.gallery_cams,
                    separate_camera_set=False,
                    single_gallery_shot=False,
                    first_match_break=True)
            m_ap = mean_ap(
                dist, self.dataset.query_pids, self.dataset.gallery_pids, self.dataset.query_cams, self.dataset.gallery_cams)
